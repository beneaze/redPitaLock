...
float pid_update(pid_state_t *pid, float setpoint, float measurement, float dt,
    ...
    /* Derivative on *measurement* (not on error): avoids amplifying ADC noise
     * at 1 kHz, which otherwise makes Kd act like a strong brake and caps
     * the output well below out_max even when P/I still need more drive.
     * D = -Kd * d(meas)/dt  in the error frame (setpoint constant).           */
    float d_term = 0.0f;
    if (dt > 0.0f) {
        if (!pid->deriv_init) {
            ...
            pid->prev_measurement = measurement;
            d_term = -error_sign * pid->kd * d_meas;
        }
    }
